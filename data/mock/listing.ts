export default {
  result: [
    {
      pic: [
        "https://images.prismic.io/99-content/dc1594fb-f413-44ff-b8b5-c3ba6cd539cd_vbWkNuxt2hjgidVttoMRDX.jpg?auto=compress,format&w=2000&q=100",
        "https://images.prismic.io/slicemachine-blank/dcea6535-f43b-49a7-8623-bf281aaf1cb2_roller-skating.png?auto=compress,format&w=2000&q=100",
      ],
      title: "Forett At Bukit Timah",
      address: "70 Anchorvale Crescent · D21",
      project_type: "Condo",
      year: 2021,
      ownership_type: "Freehold",
      psf_min: 2609,
      psf_max: 3043,
      subprice_label: "Prices from $1.99M",
      availabilities_label: "231 units · 1, 2, 3, 4 bedrooms",
      description:
        "Phone number: 8234 5678\n\nSuperb 3 bedroom in sentosa for a very affordable price and a very nearby bus station.\n\nPlease call Alex B (CEA No: R0123456) @  82345678 or the owner (83456789) to arrange for a viewing.",
    },
    {
      pic: [
        "https://images.prismic.io/slicemachine-blank/dcea6535-f43b-49a7-8623-bf281aaf1cb2_roller-skating.png?auto=compress,format&w=2000&q=100",
        "https://images.prismic.io/99-content/dc1594fb-f413-44ff-b8b5-c3ba6cd539cd_vbWkNuxt2hjgidVttoMRDX.jpg?auto=compress,format&w=2000&q=100",
      ],
      title: "Forett At Bukit Timah 2",
      address: "70 Anchorvale Crescent · D21",
      project_type: "Condo",
      year: 2021,
      ownership_type: "Freehold",
      psf_min: 2609,
      psf_max: 3043,
      subprice_label: "Prices from $1.99M",
      availabilities_label: "231 units · 1, 2, 3, 4 bedrooms",
      description:
        "Phone number: 8332 5678\n\nSuperb 3 bedroom in sentosa for a very affordable price and a very nearby bus station.\n\nPlease call Alex B (CEA No: R0123456) @  83335678 or the owner (82226789) to arrange for a viewing.",
    },
  ],
};